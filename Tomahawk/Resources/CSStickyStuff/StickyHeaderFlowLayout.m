...
NSString *const StickyHeaderParallaxHeader = @"StickyHeaderParallexHeader";
static const NSInteger kHeaderZIndex = 1024;

@interface StickyHeaderFlowLayout (Debug)
...
- (UICollectionViewLayoutAttributes *)finalLayoutAttributesForDisappearingSupplementaryElementOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)elementIndexPath {
    ...
    if ([elementKind isEqualToString:StickyHeaderParallaxHeader]) {
        StickyHeaderFlowLayoutAttributes *attribute = (StickyHeaderFlowLayoutAttributes *)[self layoutAttributesForSupplementaryViewOfKind:elementKind atIndexPath:elementIndexPath];
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!attributes && [kind isEqualToString:StickyHeaderParallaxHeader]) {
        attributes = [StickyHeaderFlowLayoutAttributes layoutAttributesForSupplementaryViewOfKind:kind withIndexPath:indexPath];
    }
    return attributes;
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForDecorationViewOfKind:(NSString *)decorationViewKind atIndexPath:(NSIndexPath *)indexPath {
    ...
    layoutAttributes.frame = CGRectMake(inset, (indexPath.row + 1) * self.itemSize.height + indexPath.row * self.minimumLineSpacing + self.parallaxHeaderReferenceSize.height + self.headerReferenceSize.height, self.collectionViewContentSize.width, self.minimumLineSpacing);
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    if (self.collectionView.dataSource != nil) {
        ...
        if (visibleParallexHeader && ! CGSizeEqualToSize(CGSizeZero, self.parallaxHeaderReferenceSize)) {
            StickyHeaderFlowLayoutAttributes *currentAttribute = [StickyHeaderFlowLayoutAttributes layoutAttributesForSupplementaryViewOfKind:StickyHeaderParallaxHeader withIndexPath:[NSIndexPath indexPathWithIndex:0]];
...
- (CGSize)collectionViewContentSize {
    // If not part of view hierarchy then return CGSizeZero (as in do).
...
+ (Class)layoutAttributesClass {
    return [StickyHeaderFlowLayoutAttributes class];
}
...
- (void)updateParallaxHeaderAttribute:(StickyHeaderFlowLayoutAttributes *)currentAttribute {
...
@implementation StickyHeaderFlowLayoutAttributes (Debug)

- (NSString *)description {
    NSString *indexPathString = [NSString stringWithFormat:@"{%ld, %ld}", (long)self.indexPath.section, (long)self.indexPath.item];

    NSString *desc = [NSString stringWithFormat:@"<StickyHeaderFlowLayout: %p> indexPath: %@ zIndex: %ld valid: %@ kind: %@", self, indexPathString, (long)self.zIndex, [self isValid] ? @"YES" : @"NO", self.representedElementKind ?: @"cell"];
...
- (BOOL)isValid {
    switch (self.representedElementCategory) {
        ...
        case UICollectionElementCategorySupplementaryView:
            if ([self.representedElementKind isEqualToString:StickyHeaderParallaxHeader]) {
                return YES;
...
@implementation StickyHeaderFlowLayout (Debug)

- (void)debugLayoutAttributes:(NSArray *)layoutAttributes {
    __block BOOL hasInvalid = NO;
    [layoutAttributes enumerateObjectsUsingBlock:^(StickyHeaderFlowLayoutAttributes *attr, NSUInteger idx, BOOL *stop) {
    ...
    if (hasInvalid) {
        NSLog(@"StickyHeaderFlowLayout: %@", layoutAttributes);
    }
}
